                                                ...
                                                <TableCell colSpan={4} className="p-0 bg-secondary/10">
                                                    <div className="p-4 space-y-3">
                                                                        ...
                                                                        filteredItems.map((item, idx) => (
                                                                            <TableRow key={idx}>
                                                                                ...
                                                                                <TableCell className="text-right font-mono font-semibold">
                                                                                    {formatCurrency(item.importo_provvigione)}
                                                                                </TableCell>
                                                                            </TableRow>
                                                                        ))
                                                                    ) : (
                                                        ...
                                                        <div className="flex items-center justify-between text-xs text-muted-foreground">
                                                            <span>
                                                                {filterQuery && filteredItems.length !== sellerData.dati_provvigione.length
                                                                    ? `${t("showing")} ${filteredItems.length} ${t("of")} ${sellerData.dati_provvigione.length}`
                                                                    : `${sellerData.dati_provvigione.length} ${t("records").toLowerCase()}`}
                                                            </span>
                                                            {filteredItems.length > 0 && (
                                                                <span className="font-mono">
                                                                    {t("total")}: {formatCurrency(
                                                                        filteredItems.reduce((sum, item) => sum + (item.importo_provvigione || 0), 0)
                                                                    )}
                                                                </span>
                                                            )}
                                                        </div>
