 ...
 */
export async function bulkImportAgenti(
  rows: AgentCsvRow[]
): Promise<BulkAgentImportResult> {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const agentData: CreateAgentRequest = {
      ...
      gettone_residenziale_standard: parseNumericField(
        row.gettone_residenziale_standard
      ),
      gettone_residenziale_bonus: parseNumericField(
        row.gettone_residenziale_bonus
      ),
      gettone_residenziale_malus: parseNumericField(
        row.gettone_residenziale_malus
      ),
      rinnovo_residenziale: parseNumericField(row.rinnovo_residenziale),
      gettone_business_standard: parseNumericField(
        row.gettone_business_standard
      ),
 ...
 */
export async function importLiquidazioni(
  data: Liquidazione[]
): Promise<ImportLiquidazioniResponse> {
 ...
 */
export async function calcolaProvvigioni(
  data: CalcoloInput[]
): Promise<CalcoloResult> {
 ...
 */
export function validateLiquidazioni(
  data: Liquidazione[]
): { valid: Liquidazione[]; invalid: number } {
